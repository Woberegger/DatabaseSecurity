...
```bash
$CONTAINERCMD pull mongodb/mongodb-community-server
```
...
>$CONTAINERCMD run --name mongodb --network export NETWORK=my-docker-network -d <br>
...
$CONTAINERCMD-compose -f 09b_mongodb-docker-compose.yaml up -d
```
...
```bash
$CONTAINERCMD start mongoapp
$CONTAINERCMD logs mongoapp
```