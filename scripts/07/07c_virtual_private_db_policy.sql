-- connect to database as sys user, e.g. docker exec -it -u oracle Oracle23Free sqlplus 'JONES/jones1234@IMS'
...
CREATE OR REPLACE PACKAGE BODY context_package IS
  PROCEDURE set_context IS
    ...
    v_id     NUMBER;
  BEGIN
    DBMS_SESSION.set_context('JONES','SETUP','TRUE');
    ...
    BEGIN
      ...
      DBMS_SESSION.set_context('JONES','DEPT_ID', v_id);
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        DBMS_SESSION.set_context('JONES','DEPT_ID', 0);
    END;
    
    DBMS_SESSION.set_context('JONES','SETUP','FALSE');
  END set_context;
END context_package;
/
...
connect sys/FhIms9999 as sysdba
--
...
CREATE OR REPLACE CONTEXT JONES USING context_package;
--
BEGIN
